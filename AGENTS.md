...
```
annatto/<qualified suite or class>#<test name>     code tests
annatto/<data path>[#<case id>]                    data-driven cases: the file's `id` field
```

Surveyor's `scripts/compare-test-ids.py` diffs those IDs against `tests/coverage/annatto.tsv`
...
- Expectation files: `src/test/resources/<eco>/<name>-<version>-expected.json`, paired with
  `test-corpus/<eco>/<name>-<version>.<ext>` (downloaded, never committed). The first key is
  `"id": "<eco>/<name>-<version>"`; `SourceOfTruthLoader` requires it and
  `ExpectedJsonIdTest` checks every file without a corpus.
- Through `spice`, goatrodeo's Annatto strategy exposes name, version, description, license,
  publisher and `Annatto:Ecosystem` plus the purl; **dependencies are not observable**, so a
  dependency assertion is unit-only. If you add a field that goatrodeo emits, tell Surveyor's
  `adapters/annatto.py` maintainer so it is checked there too.
